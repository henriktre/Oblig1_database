...
class UnitTests extends TestCase
{
	...
	*/
	...
	public function testAddBook()
	{
		...
		for ($i = self::FIXTURE_SIZE; $i < sizeof(self::$TEST_CASES); $i++)
		{
			...
			if (self::$TEST_CASES[$i][self::OUTCOME_IDX] == self::OUTCOME_SUCCESS)
			{
				...
				// Was the book added?
				$this->assertEquals($dbSize + 1, $this->getConnection()->getRowCount(TEST_TABLE_NAME));
				$dbSize ++;
	...
	public function testDeleteBook()
	{
		...
		$this->assertEquals($dbSize, $this->getConnection()->getRowCount(TEST_TABLE_NAME), "Expecting book table size to decrement");
		...
		$this->assertEquals($dbSize, $this->getConnection()->getRowCount(TEST_TABLE_NAME), "Invalid book ID: Expecting book table size to decrement");
	}
